...
pushd "${ansible_dir}"
if [[ -n "${tags-}" ]]; then
    ansible-playbook main.yml --ask-pass --tags "${tags}" --extra-vars '{"allow_reboot": '\""${allow_reboot}"\"', "install_intel_vtd": '\""${install_intel_vtd}"\"' }' --limit "${host}"
else
    ansible-playbook main.yml --ask-pass --extra-vars '{"allow_reboot": '\""${allow_reboot}"\"', "install_intel_vtd": '\""${install_intel_vtd}"\"' }' --limit "${host}"
fi
popd
